...
func TestBalancerPick(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		result, err := picker.Pick(balancer.PickInfo{Ctx: ctx2})
		if err != nil {
			t.Fatalf("picker.Pick failed!idx:=%d", i)
		}
		sc := result.SubConn.(*testSubConn)
...
func Benchmark_Wrr(b *testing.B) {
	...
	for i := 0; i < 50; i++ {
		...
		pbi.ReadySCs[&testSubConn{addr:addr}] = base.SubConnInfo{Address: addr}
	}
	wpb := &p2cPickerBuilder{}
	picker := wpb.Build(pbi)
...
func newController(svrNum int, cliNum int) *controller {
	...
	for _, v := range servers {
		//scs[v.addr] = v
		pbi.ReadySCs[v] = base.SubConnInfo{Address: v.addr}
	}
	for i := 0; i < cliNum; i++ {
		wpb := &p2cPickerBuilder{}
		picker := wpb.Build(pbi)
